    ...
    return (
      <section className="wallet-all-page">
        <section className="wallet-ledger-filter">
          <label><LineIcon name="search" label="搜索" className="filter-icon" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索订单号 / TxID / 备注" /></label>
  ...
  return (
    <section className="wallet-dashboard">
      ...
      <form className="wallet-recharge-card">
        <h3>充值</h3>
        <label>充值金额<div className="wallet-amount-field"><input min="1" step="0.01" type="number" value={amount} onChange={(event) => setAmount(event.target.value)} /><b>USDT</b></div></label>
        <div className="wallet-method-block">
          <span>充值方式</span>
          <div className="wallet-methods">
            <WalletMethod value="alipay" title="支付宝" desc="即时到账，推荐使用" active={method === "alipay"} onClick={() => setMethod("alipay")} />
            <WalletMethod value="usdt_trc20" title="USDT-TRC20" desc="链上转账，低手续费" active={method === "usdt_trc20"} onClick={() => setMethod("usdt_trc20")} />
          </div>
        </div>
        <button className="wallet-submit" disabled={submitting} onClick={createRecharge} type="button">{submitting ? "提交中..." : "立即充值"}</button>
        <small><LineIcon name="warning" label="提示" className="wallet-note-icon" /> 选择支付方式，系统会按实时汇率结算人民币支付金额。</small>
      </form>
